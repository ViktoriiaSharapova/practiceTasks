public class Task {
    public static void main(String[] args) {
        double age = 18;
        boolean hasLearningPermit = true;
        boolean hasDriverLicense = true;

        // ----Do not change below lines. needed for testing---
        age = args.length > 0 ? Double.parseDouble(args[0]) : age;
        hasLearningPermit = args.length > 1 ? Boolean.parseBoolean(args[1]) : hasLearningPermit;
        hasDriverLicense = args.length > 2 ? Boolean.parseBoolean(args[2]) : hasDriverLicense;
        //-----------------------------------------------------

        //Write your solution here
        boolean allowedToDrive = (age > 18 && hasDriverLicense) || (age >= 15.5 && hasLearningPermit);
        System.out.println("Is Mike allowed to drive? " + allowedToDrive);

    }
}